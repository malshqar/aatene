@extends('dashboard::index',['title'=>"تعديل بيانات $admin->name"]);
@section('content')
    <!--begin::Main-->
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        ...
        <div class="d-flex flex-column flex-column-fluid">
            <!--begin::Toolbar-->
            @include('shared::layouts.components.elements.toolbar', ['back_url' => 'dashboard.admins.index', 'previews' => 'قائمة المدراء', 'current' => "تعديل بيانات $admin->name"])
            <!--end::Toolbar-->
            <!--begin::Content-->
            <div id="kt_app_content" class="app-content flex-column-fluid">
                ...
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form class="form d-flex flex-column flex-lg-row"
                        ...
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="admin_id" value="{{ $admin->id }}" id="">
                        <!--begin::Aside column-->
                            ...
                            <div class="card card-flush py-4 ">
                                ...
                                <div class="card-header border-0 ">
                                    <h3 class="card-title align-items-start flex-column">
                                        <span class="card-label fw-bold fs-3 mb-1"> تعديل بيانات {{ $admin->name }}</span>
                                    </h3>
                                </div>
                                <!--end::Header-->
                                <!--begin::Body-->
                                <div class="card-body py-3">
                                    <div class="mb-5">
                                        @include(
                                            ...
                                            ['name' => 'name', 'lable' => 'اسم المدير', 'placeholder' => 'أدخل اسم المدير', 'value' => $admin->name]
                                        )
                                    </div>
                                    <div class="mb-5">
                                        @include(
                                            'shared::layouts.forms.inputs.input-with-lable',
                                            ['name' => 'email', 'lable' => 'بريد المدير', 'placeholder' => 'أدخل بريد المدير', 'value' => $admin->email]
                                        )
                                    </div>
                                    <div class="mb-5">
                                        @include(
                                            'shared::layouts.forms.inputs.input-with-lable',
                                            ['name' => 'phone_number','id'=>'phone_number', 'lable' => 'رقم هاتف المدير', 'placeholder' => 'أدخل رقم هاتف المدير', 'value' => $admin->phone_number]
                                        )
                                        <!--begin::Hint-->
                                        <div class="text-muted">
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <select data-control="select2" name="status" @class([
                                            ...
                                            <option value="active" @selected(old('status', $admin->status) == 'active')>حساب
                                                فعال</option>
                                            <option value="inactive" @selected(old('status', $admin->status) == 'inactive')>حساب
                                                غير فعال
                                            </option>
                                        </select>
                                    </div>
