@extends('dashboard::index', ['title' => __("أقسام الأسئلة الشائعة")])
...
@section('content')
    ...
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        ...
        <div class="d-flex flex-column flex-column-fluid">
            <!--begin::Toolbar-->
            @include('shared::layouts.components.elements.toolbar', ['back_url' => 'dashboard.index', 'previews' => 'الرئيسية', 'current' => __("أقسام الأسئلة الشائعة")])
            <!--end::Toolbar-->
            <!--begin::Content-->
                    ...
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-header border-0 pt-5">
                            <div class="card-toolbar">
                                @can('user.create')
                                    <a href="{{ route('dashboard.faqs_categories.create') }}"
                                        class="btn btn-sm btn-light-primary fs-3">
                                        <i class="ki-duotone ki-plus "></i> {{__("إضافة قسم")}} </a>
                                @endcan
                            </div>
                        </div>
                        <!--end::Header-->
                                    ...
                                    <tbody>
                                        @foreach ($categories as $category)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="fs-5 fw-bold">{{ \Carbon\Carbon::parse( $category->created_at)->format('Y\/m\/d') }}</span>
                                                </td>
                                                ...
                                                @canany(['user.edit', 'user.ban', 'user.delete'])
                                                    ...
                                                    <td class="text-end">
                                                        ...
                                                        @can('user.edit')
                                                            <a href="{{ route('dashboard.faqs_categories.edit', $category->id) }}"
                                                        ...
                                                        @can('user.delete')
                                                            <a onclick="confirmDestroy('{{ route('dashboard.faqs_categories.destroy', $category->id) }}', this)"
